...
class AlphaMixSampling(Strategy):
    def __init__(self, X, Y, idxs_lb, model, handler, args):
        super(AlphaMixSampling, self).__init__(X, Y, idxs_lb, model, handler, args)
    ...
    def find_alpha(self):
        assert self.alpha_num_mix <= self.n_label - (
