...
class PoincareKmeansUncertaintySampling(Strategy):  # HEAl2
    def __init__(self, X, Y, idxs_lb, net, handler, args):
...
class PoincareKmeansSamplingNew(Strategy):
    ...
    def init_centers_hyp(self, X, K):
        ind = np.argmin(
            [self.manifold.norm(torch.tensor(s.clone().detach()), self.curvature) for s in X.clone().detach()])
        mu = [X[ind]]
        indsAll = [ind]
        centInds = [0.] * len(X)
        cent = 0
    ...
    def query(self, n):
        ...
        # all_emb = (all_emb / max(self.manifold.norm(all_emb,self.curvature)))
        all_emb_norm = self.manifold.norm(all_emb,
                                          self.curvature)  # [self.manifold.norm(s, self.curvature) for s in all_emb]
        # fit unsupervised clusters and plot results
        print('Running Kmean++ on 1D norm ...')
        idxs_unlabeled = np.arange(self.n_pool)[~self.idxs_lb]
        chosen_uncertainty = self.init_centers(all_emb_norm[idxs_unlabeled], n - int(
            3 * n / 4) + 10)  # np.argsort(all_emb_norm[idxs_unlabeled].numpy())[:n-int(3*n/4)+10].tolist() #
        chosen_diversity = self.init_centers_hyp(all_emb[idxs_unlabeled], int(3 * n / 4))
        chosen = random.sample(set(chosen_uncertainty).union(set(chosen_diversity)),
                               n)  # np.concatenate([chosen_diversity,chosen_uncertainty]).tolist()#list(set(chosen_uncertainty.tolist()).union(set(chosen_diversity)))
        # print(len(chosen))
        plt.scatter(all_emb_norm[idxs_unlabeled], np.zeros_like(all_emb_norm[idxs_unlabeled]), c=self.Y[idxs_unlabeled],
                    s=2, cmap='Spectral')
        plt.scatter(all_emb_norm[idxs_unlabeled[chosen_uncertainty]],
                    np.zeros_like(all_emb_norm[idxs_unlabeled[chosen_uncertainty]]),
                    c=self.Y[idxs_unlabeled[chosen_uncertainty]], edgecolor='black', linewidth=0.3, marker='*',
                    cmap='Spectral')
...
class UmapPoincareKmeansSampling(Strategy):
    ...
    def query(self, n):
        ...
        df = pd.DataFrame(np.concatenate(
            [np.expand_dims((self.Y[idxs_unlabeled[chosen]]).numpy(), axis=1),
             np.expand_dims(idxs_unlabeled[chosen], axis=1),
...
class HyperNorm_plus_RiemannianBadge_Sampling(Strategy):
    ...
    def train(self, reset=True, optimizer=0, verbose=True, data=[], net=[], model_selection=None):
        super(HyperNorm_plus_RiemannianBadge_Sampling, self).train(reset, optimizer, verbose, data, net,
                                                                   model_selection)
...
class HypNetEmbeddingPoincareKmeansSampling(Strategy):
    ...
    def query(self, n):
       ...
       """
        ...
        embedding = self.get_hyperbolic_embedding(self.X, self.Y).numpy()
        chosen = self.init_centers_hyp(torch.tensor(embedding[idxs_unlabeled]),
                                       n)  # note the only difference with hyperBadge
        self.save_images_and_embeddings(embedding, idxs_unlabeled, chosen, self.iteration_ind)
        self.iteration_ind += 1
        return idxs_unlabeled[chosen]
